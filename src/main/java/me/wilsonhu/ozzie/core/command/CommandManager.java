...
public class CommandManager extends AbstractManager {
    ...
    private final List<Command> pluginCommands;
    private final List<String> categoryList;

    public CommandManager(Ozzie ozzie) {
        super(ozzie);
        info("Building CommandManager...");
    ...
    public void onCommandValidator(List<Command> list, String[] s, String full, MessageReceivedEvent event) {
        for (Command c : list) {
    ...
    public void addCommands(PluginModule pl) {
        for (Command cmd : pl.getPlugin().getCommands()) {
            ...
            info(String.format("[%s] Loading command %s", pl.getSchema().getName(), cmd.getNames()[0]));
        }
        for (Command c : getPluginCommands()) {
            String category = c.getAsCategory().toLowerCase().trim();
